...
export default function About() {
  ...
  const [useBtn, setuseBtn] = useState("Enable Dark Mode");
  const darkMode = () => {
    if (myStyle.backgroundColor === 'white') {
      setmyStyle(dark);
      setuseBtn("Enable Light Mode");
    } else {
      setmyStyle(light);
      setuseBtn("Enable Dark Mode");
    }
  };
  return (
    <>
      ...
      <button className="btn btn-primary my-2" onClick={darkMode}>
        {useBtn}
      </button>
    </>
  );
